...
namespace ECommerce.Services
{
    ...
    public class ProductService
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(p.Name))    throw new ArgumentException("Emri nuk mund të jetë bosh!");
            if (string.IsNullOrWhiteSpace(p.StoreId)) throw new ArgumentException("StoreId required.");
            if (p.Price <= 0)                          throw new ArgumentException("Çmimi duhet të jetë > 0!");
