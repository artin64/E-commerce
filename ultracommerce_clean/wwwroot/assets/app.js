...
async function reloadCurrentVendor() {
    ...
    const [storefront, admin] = await Promise.all([
        fetchJson(`/api/storefront?${params.toString()}`),
        fetchJson(`/api/admin/dashboard?vendorId=${encodeURIComponent(state.vendorId)}`)
    ]);
...
async function refreshQuote() {
    ...
    const quote = await fetchJson("/api/cart/quote", {
        method: "POST",
...
function render() {
    if (!state.bootstrap || !state.storefront || !state.admin) {
        renderLoading((state.flash && state.flash.message) || "Preparing the demo experience...");
        return;
    }
                    ...
                    <div class="admin-column">
                        <div class="panel admin-column">
                            ...
                            <form id="product-form">
                                <input type="hidden" name="productId" value="${escapeAttribute((editingProduct && editingProduct.id) || "")}" />
                                <div class="form-grid">
                                    <div class="field">
                                        <label for="product-name">Name</label>
                                        <input id="product-name" name="name" type="text" required value="${escapeAttribute((editingProduct && editingProduct.name) || "")}" />
                                    </div>
                                    <div class="field">
                                        <label for="product-category">Category</label>
                                        <select id="product-category" name="categoryId" required>
                                            ${state.admin.categories.map((category) => `
                                                <option value="${escapeAttribute(category.id)}" ${((editingProduct && editingProduct.categoryId) || (state.admin.categories[0] && state.admin.categories[0].id)) === category.id ? "selected" : ""}>
                                                    ${escapeHtml(category.name)}
                                                </option>
                                            `).join("")}
                                        </select>
                                    ...
                                    <div class="field field--full">
                                        ...
                                        <textarea id="product-description" name="description" required>${escapeHtml((editingProduct && editingProduct.description) || "")}</textarea>
                                    </div>
                                    <div class="field">
                                        <label for="product-price">Price</label>
                                        <input id="product-price" name="price" type="number" min="1" step="0.01" required value="${(editingProduct && editingProduct.price) || ""}" />
                                    </div>
                                    <div class="field">
                                        <label for="product-original-price">Original price</label>
                                        <input id="product-original-price" name="originalPrice" type="number" min="0" step="0.01" value="${(editingProduct && editingProduct.originalPrice) || ""}" />
                                    </div>
                                    <div class="field">
                                        <label for="product-stock">Stock</label>
                                        <input id="product-stock" name="stock" type="number" min="0" step="1" required value="${(editingProduct && editingProduct.stock) || 10}" />
                                    </div>
                                    <div class="field">
                                        <label for="product-tags">Tags</label>
                                        <input id="product-tags" name="tags" type="text" placeholder="new-in, travel, featured" value="${escapeAttribute((editingProduct && editingProduct.tags.join(", ")) || "")}" />
                                    </div>
                                </div>
                                <label class="checkbox-field">
                                    <input name="isFeatured" type="checkbox" ${(editingProduct && editingProduct.isFeatured) ? "checked" : ""} />
                        ...
                        <div class="panel admin-column">
                            ...
                            <form id="campaign-form">
                                <input type="hidden" name="campaignId" value="${escapeAttribute((editingCampaign && editingCampaign.id) || "")}" />
                                <div class="form-grid">
                                    <div class="field">
                                        <label for="campaign-title">Title</label>
                                        <input id="campaign-title" name="title" type="text" required value="${escapeAttribute((editingCampaign && editingCampaign.title) || "")}" />
                                    </div>
                                    <div class="field">
                                        <label for="campaign-position">Position</label>
                                        <input id="campaign-position" name="position" type="number" min="1" step="1" required value="${(editingCampaign && editingCampaign.position) || 1}" />
                                    </div>
                                    <div class="field field--full">
                                        <label for="campaign-subtitle">Subtitle</label>
                                        <textarea id="campaign-subtitle" name="subtitle" required>${escapeHtml((editingCampaign && editingCampaign.subtitle) || "")}</textarea>
                                    </div>
                                    <div class="field">
                                        <label for="campaign-cta">Call to action</label>
                                        <input id="campaign-cta" name="callToAction" type="text" value="${escapeAttribute((editingCampaign && editingCampaign.callToAction) || "Open campaign")}" />
                                    </div>
                                    <div class="field">
                                        ...
                                        <select id="campaign-tone" name="accentTone">
                                            ${["sunset", "graphite", "electric", "forest", "cream"].map((tone) => `
                                                <option value="${tone}" ${((editingCampaign && editingCampaign.accentTone) || state.admin.vendor.accentTone) === tone ? "selected" : ""}>${tone}</option>
                                            `).join("")}
                                        </select>
                                    </div>
                                </div>
...
function bindEvents() {
    ...
    document.querySelectorAll("[data-jump='products']").forEach((button) => {
    ...
    document.querySelectorAll("[data-reset-product-form]").forEach((button) => {
        button.addEventListener("click", () => {
            state.editingProductId = null;
            render();
        });
    ...
    if (campaignForm) {
        campaignForm.addEventListener("submit", async (event) => {
            ...
            const payload = {
                ...
                accentTone: String(formData.get("accentTone") || ((state.admin && state.admin.vendor.accentTone) || "sunset"))
            };
    ...
    document.querySelectorAll("[data-reset-campaign-form]").forEach((button) => {
        button.addEventListener("click", () => {
            state.editingCampaignId = null;
            render();
        });
...
function renderAdminCampaignCard(campaign) {
    return `
...
function getEditingProduct() {
    return (state.admin && state.admin.products.find((product) => product.id === state.editingProductId)) || null;
}

function getEditingCampaign() {
    return (state.admin && state.admin.campaigns.find((campaign) => campaign.id === state.editingCampaignId)) || null;
}

function createEmptyQuote(vendorId) {
    return {
        vendorId,
        currency: "EUR",
        items: [],
